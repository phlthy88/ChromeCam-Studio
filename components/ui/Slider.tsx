  ...
  return (
    <div className={`flex flex-col gap-2 ${disabled ? 'opacity-[0.38]' : ''}`}>
      ...
      <div className="flex justify-between items-center">
        <label
          id={`slider-label-${uniqueId}`}
          htmlFor={`slider-input-${uniqueId}`}
          className="md-label-large text-on-surface-variant"
        >
          {label}
        </label>
        {showValue && (
            ...
            {Array.from({ length: Math.floor((max - min) / step) + 1 }, (_, i) => (
              <div
                ...
                className={`w-1 h-1 rounded-full transition-colors duration-short2 ${
                  min + i * step <= value ? 'bg-on-primary' : 'bg-on-surface-variant'
                }`}
              />
            ))}
          </div>
