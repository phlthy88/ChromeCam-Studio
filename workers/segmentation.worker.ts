...
async function initializeSegmenter(modelPath?: string): Promise<boolean> {
    try {
        ...
        const cdnPath = modelPath || 'https://cdn.jsdelivr.net/npm/@mediapipe/selfie_segmentation@0.1.1675465747/';
        ...
        // Create segmenter
        const model = (bodySegmentation as { SupportedModels: { MediaPipeSelfieSegmentation: unknown } }).SupportedModels.MediaPipeSelfieSegmentation;
        ...
        segmenter = await (bodySegmentation as { createSegmenter: (model: unknown, config: unknown) => Promise<unknown> }).createSegmenter(model, segmenterConfig);
 ...
 */
async function processFrame(imageBitmap: ImageBitmap, width: number, height: number): Promise<Uint8ClampedArray | null> {
    ...
    try {
        ...
        // Run segmentation
        const bodySegmentation = (self as unknown as { bodySegmentation: unknown }).bodySegmentation;
        const segmentation = await (segmenter as { segmentPeople: (source: OffscreenCanvas) => Promise<unknown[]> }).segmentPeople(offscreenCanvas);

        // Convert to binary mask
        const mask = await (bodySegmentation as { toBinaryMask: (seg: unknown[], fg: unknown, bg: unknown) => Promise<ImageData> }).toBinaryMask(
            segmentation,
            FOREGROUND_COLOR,
            BACKGROUND_COLOR
        );
    ...
    switch (message.type) {
        ...
        case 'segment': {
            ...
            if (maskData) {
                ...
                (self as unknown as { postMessage: (message: unknown, transfer: Transferable[]) => void }).postMessage(response, [maskData.buffer] as Transferable[]);
            } else {
                const errorResponse: WorkerResponseError = {
                    type: 'error',
