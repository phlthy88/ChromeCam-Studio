...
// Reusable buffers to prevent GC thrashing
...
async function initSegmenter() {
  try {
    segmenter = new SelfieSegmentation({
      locateFile: (file) => `/mediapipe/${file}`, // Loads from public/mediapipe
    });
    segmenter.setOptions({ modelSelection: 1 }); // 1 = Landscape
...
function onResults(results: Results) {
  ...
  const latency = now - state.startTime;
  fpsHistory.push(1000 / latency);
  if (fpsHistory.length > historySize) fpsHistory.shift();
  const avgFps = fpsHistory.reduce((a, b) => a + b, 0) / fpsHistory.length;

  // Normalize Mask (Draw to OffscreenCanvas then transfer new bitmap)
  ...
  inputBitmap.close(); // Critical cleanup
}
...
async function processFrame(id: number, bitmap: ImageBitmap, autoFrame: boolean) {
  ...
  try {
    // MediaPipe needs a canvas source, not raw bitmap
